...
type LibraryTab = 'SONGS' | 'QUEUE' | 'ALBUMS' | 'ARTISTS' | 'FOLDERS';
...
export default function Library({
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-900/60 border border-zinc-800 rounded-2xl overflow-hidden p-5 backdrop-blur-md">
      ...
      {/* MAIN LAYOUT: Tabs */}
      <div className="flex bg-zinc-950 p-1 rounded-xl border border-zinc-800/80 mb-4 text-xs font-mono overflow-x-auto scrollbar-none">
        {(['SONGS', 'QUEUE', 'ALBUMS', 'ARTISTS', 'FOLDERS'] as LibraryTab[]).map(tab => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`flex-1 py-1.5 px-3 rounded-lg text-center font-bold tracking-wider transition-all cursor-pointer whitespace-nowrap ${activeTab === tab ? 'bg-pink-600 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            {tab}
          </button>
        ))}
        ...
        {activeTab === 'QUEUE' && (
          <div className="space-y-1.5 text-left animate-fade-in">
            <div className="flex items-center justify-between pb-2 mb-2 border-b border-zinc-800/40">
              <span className="text-[10px] font-mono text-zinc-400 font-bold tracking-wider uppercase flex items-center space-x-1.5">
            ...
            {queue.length > 0 ? (
              queue.map((track, index) => {
                ...
                return (
                  ...
                  >
                    {/* Index, Cover Art, and Info */}
                    <div 
                      onClick={() => onSelectTrack(track)}
                      className="flex items-center space-x-3 flex-1 min-w-0 cursor-pointer"
                    >
                      <span className="text-[9px] font-mono font-bold text-zinc-500 w-4 text-center shrink-0">
                        {index + 1}
                      </span>
                      <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${track.coverColor || 'from-pink-500 to-purple-600'} flex items-center justify-center shadow-inner shrink-0`}>
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-[10px] text-zinc-600 mt-1 font-mono">Tracks will appear here as you play</p>
              </div>
            )}
          </div>
        )}
